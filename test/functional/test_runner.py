...
`test/functional/test_framework/test_framework.py:TrollcoinTestFramework.main`.
...
ZMQ_SCRIPTS = [
    # ZMQ test can only be run if trollcoin was built with zmq-enabled.
...
def main():
    ...
    enable_trollcoind = config["components"].getboolean("ENABLE_BITCOIND")
    enable_zmq = config["components"].getboolean("ENABLE_ZMQ") and not args.nozmq

    if config["environment"]["EXEEXT"] == ".exe" and not args.force:
        # https://github.com/trollcoin/trollcoin/commit/d52802551752140cf41f0d9a225a43e84404d3e9
        # https://github.com/trollcoin/trollcoin/pull/5677#issuecomment-136646964
        print("Tests currently disabled on Windows by default. Use --force option to enable")
        sys.exit(0)

    if not (enable_wallet and enable_utils and enable_trollcoind):
        print("No functional tests to run. Wallet, utils, and trollcoind must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, jobs=1, enable_coverage=False, args=[]):
    ...
    if "BITCOIND" not in os.environ:
        os.environ["BITCOIND"] = build_dir + '/src/trollcoind' + exeext
...
class TestHandler:
    ...
    def __init__(self, num_tests_parallel, tests_dir, test_list=None, flags=None):
        ...
        self.flags = flags
        self.num_running = 0
        # In case there is a graveyard of zombie trollcoinds, we can apply a
...
class RPCCoverage(object):
    ...
    commands per `trollcoin-cli help` (`rpc_interface.txt`).
