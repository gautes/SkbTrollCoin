...
###   This script attempts to download the signature file SHA256SUMS.asc from trollcoin.org
...
WORKINGDIR="/tmp/trollcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
BASEDIR="https://trollcoin.org/bin/"
VERSIONPREFIX="trollcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'trollcoin-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Trollcoin Core binary release signing key installed?"
   fi
