#!/bin/bash
# Copyright (c) 2013 The Trollcoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Trollcoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Trollcoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Trollcoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Trollcoin 0.7 datadir."
    ;;
  3)
    echo "Detected Trollcoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Trollcoin 0.8 datadir."
    ;;
esac
