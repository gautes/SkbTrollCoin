...
*Setup instructions for a Gitian build of Trollcoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Trollcoin
...
- [Building Trollcoin Core](#building-trollcoin-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-trollcoin-host/README.md).
...
Clone the git repositories for trollcoin and Gitian.
...
in the trollcoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Trollcoin Core
----------------

To build Trollcoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the trollcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/trollcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/trollcoin/trollcoin
...
For example:
```bash
URL=https://github.com/laanwj/trollcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit trollcoin=${COMMIT} --url trollcoin=${URL} ../trollcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit trollcoin=${COMMIT} --url trollcoin=${URL} ../trollcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit trollcoin=${COMMIT} --url trollcoin=${URL} ../trollcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the trollcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../trollcoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/trollcoin
SIGPATH=/some/root/path/trollcoin-detached-sigs

./bin/gbuild --url trollcoin=${BTCPATH},signature=${SIGPATH} ../trollcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/trollcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/trollcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/trollcoin-osx-build.assert
```
...
[trollcoin-core/gitian.sigs](https://github.com/trollcoin-core/gitian.sigs/) repository, or if that's not possible create a pull
